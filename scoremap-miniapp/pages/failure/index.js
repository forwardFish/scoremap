if (typeof Page === 'function') {
  ...
  createReplicaPage({
    ...
    derived: false,
    hotspots: [
      { action: 'retry', label: '重新开始分析', className: 'failure-retry' },
      { action: 'reupload', label: '重新上传资料', className: 'failure-reupload' },
      { action: 'home', label: '返回首页', className: 'failure-home' }
    ],
    actions: {
      retry: { url: '/pages/analysis/index' },
      reupload: { url: '/pages/index/index' },
  ...
  function createFailurePageState(client = createMiniappApiClient(), options = {}) {
    const orderId = options.orderId || 'order-v143-10-failure';
    ...
    const page = {
      ...
      retryAnalysis() {
        const response = client.request('POST', `/api/diagnosis-orders/${orderId}/start-preview-analysis`, {
          taskId: `task-v143-10-retry-${orderId}`,
          retry: true,
          progress: 10,
