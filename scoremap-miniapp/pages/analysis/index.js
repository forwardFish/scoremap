if (typeof Page === 'function') {
  ...
  createReplicaPage({
  "title": "AI 正在分析",
  "reference": "/assets/reference/analysis.jpg",
  "derived": false,
  "hotspots": [
    {
      "action": "later",
      "label": "稍后查看",
      "className": "analysis-later"
    },
    {
      "action": "refresh",
      "label": "刷新进度",
...
function createAnalysisPageState(client = createMiniappApiClient(), options = {}) {
  const orderId = options.orderId || 'order-t08-analysis';
  ...
  const page = {
    ...
    orderId,
    uiReference: {
      png: 'docs/UI/小程序/AI分析.png',
      stitch: 'docs/UI/小程序/stitch_codex_development_blueprints/ai_1/screen.png'
    },
    title: 'AI 正在分析中',
    ...
    loadProgress({ source, elapsedMs }) {
      const response = client.request('GET', `/api/diagnosis-orders/${orderId}/analysis-progress`, { source, elapsedMs });
      lastProgress = normalizeProgress(response.body);
      toast = null;

      if (lastProgress.status === 'preview_done' || lastProgress.progress >= 100) {
        return {
          status: 'READY',
      ...
      if (lastProgress.status === 'preview_failed' || lastProgress.status === 'failed') {
        return {
          status: 'FAILED',
    ...
    laterView() {
      const response = client.request('GET', '/api/my/reports', { source: 'analysis-later-view', orderId });
...
function normalizeProgress(body) {
  return {
    ...
    progress: Number.isFinite(body.progress) ? body.progress : 0,
...
module.exports = { createAnalysisPageState };
