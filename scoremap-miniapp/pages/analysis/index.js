if (typeof Page === 'function') {
  ...
  createReplicaPage({
    ...
    derived: false,
    hotspots: [
      { action: 'later', label: '稍后查看', className: 'analysis-later' },
      { action: 'refresh', label: '刷新进度', className: 'analysis-refresh' }
    ],
    actions: {
      later: { url: '/pages/reports/index' },
      refresh: { url: '/pages/preview/index' }
    },
    cards: []
  });
} else {
  ...
  function createAnalysisPageState(client = createMiniappApiClient(), options = {}) {
    const orderId = options.orderId || 'order-v143-10-analysis';
  ...
  module.exports = { DEFAULT_STEPS, createAnalysisPageState };
}
