...
## 实现功能
实现 POST /api/payments/create、POST /api/payments/wechat/callback 的本地模拟；校验 basic/full 权益、幂等、取消、失败、补偿恢复；不得调用真实微信支付。
...
## 允许改动范围
server/src/routes/**, server/src/services/**, server/test/payment*.js
...
## 验收标准
- 当前任务主验收面 backend-api-payment 已实现或已验证。
...
- docs/auto-execute/evidence/backend-api-payment/
